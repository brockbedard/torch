...
## Version
**v0.34.0 "Football Integrity"** — Core engine logic audit & UI balance: Fixed "multiple TD" bug via explicit state reset after scoring, implemented Safeties (2pts + free kick from 20), added Overtime support (matching possessions), updated Kneel to cost a down, fixed kickoff return TD points. UI: Redesigned Opponent TD overlay (impactful disappointment, red flash, screen shake), refined gameplay vertical spacing using 8px units, reduced scoreboard down/dist font to 11px, synchronized score/torch animations to trigger after overlay fade, refined Coin Toss results (one-line title, text-only options). Fixes: TORCH point allocation bug on turnovers (defense now correctly credited).
