...
## Version
**v0.40.1 "Ember Eight"** — polish + ship-blocker pass on top of v0.40. Pregame refactored into three independent flex-centered phase groups (coin · decision · face-down) so the coin toss / card pick / reveal screens sit vertically centered on any phone height instead of anchored to the top. Kickoff result overlay rebuilt around the team's signature wordmark (Italiana for Dolphins, Zilla Slab for Boars, etc.) with a team badge, gold "KICKOFF" eyebrow, team-colored result card, and staggered broadcast entrance. Toss result stamp unified across user-win and AI-win paths. Scorebug tap fixed in 2H (removed stuck `phase === 'busy'` guard). Null guards on `snapResolver`/`turnoverReturns`; `getTeam()` returns loud `UNKNOWN_TEAM` sentinel instead of silently mapping to the first team on stale saves. Discard rapid-tap guard, conversion double-tap guard, SNAP-during-torch "WAIT" grayscale state, tooltip touchmove/touchcancel cancel, bottom `safe-area-inset` on dailyDrive/pregame/seasonRecap/settings, shop sheet migrated to GSAP, audio `red_zone`/`goal_to_go` crowd-intensity states wired into the settle logic, `endGame.js` career/prompt storage moved to the `storage.js` facade.
