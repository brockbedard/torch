...
## Version
**v0.35.0 "Moments of Joy"** — 8 game-feel systems: pitch-shifted synergy combo pops (MATCHUP/TEMPO/HOT READ/CARD COMBO badges with ascending 12-TET audio), Glow Tell shop card reveal (face-down glow → shimmer reveal), Walkout TD card (featured player flies in post-touchdown), Shame Card on INT/fumble (desaturate + rotated verdict stamp), Hitstop 2.0 sack block (T-rot-shake, bass drop, bigHit haptic), Pressure Heartbeat SNAP button (double-thump animation + audio on high-pressure downs), Player Momentum Boosts (spikeMomentum/crashMomentum on TDs/INTs/fumbles/sacks, HOT badge at level 4+), Big Play Interface (game-icons.net SVGs for all event types). Game speed locked to 1.0 (settings speed section removed). Standalone stakeholder demo page (joy-demo.html).
